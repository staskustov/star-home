...
`ops.audit[]`: `actorUserId`, `companyId`, `objectId`, `action`, `target`, `result`, `error`, `at`.

Пишутся только: открытие ворот, пропуск, заявка, статус заявки, оплата, вызов охраны.

Не пишутся: вход, изменения каталога, жители, настройки режимов, отказы в доступе. Нет роли, IP, устройства. Журнал лежит в изменяемом снапшоте ops.
...
- У записи нет поля `unitId` (журнал до Phase E) — место неизвестно, сотруднику корпуса запись не показывается.
