...
- `/security/cameras` — стена камер в отдельном окне браузера. Кнопка «Отдельное окно» на посту открывает одно именованное окно: повторное нажатие переводит его на выбранный объект, а не плодит окна. Если у компьютера несколько мониторов и браузер поддерживает Window Management API, окно после разрешения переносится на второй экран. Есть полноэкранный режим. Окно получает только камеры (`securityCameras`), кадр запрашивается по id камеры; выведенная из работы или неисправная камера отвечает 409.
