...
С Phase 6–11 первый контур работает внутри Next.js: файл операций, локальный адаптер устройств и расчётный провайдер оплаты. Внешний банк, телефонный push и внешняя языковая модель не подключены. Ядро по-прежнему не выбирает производителя устройства и не вызывает банк напрямую.
