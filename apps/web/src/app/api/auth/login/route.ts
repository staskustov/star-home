...
export async function POST(request: Request) {
  ...
  const result = await rpcWith<{ userId?: string; membershipId?: string | null; redirectTo?: string; message?: string }>(null, "login", body);
  ...
  response.cookies.set(sessionCookie, signSession(result.body.userId, result.body.membershipId ?? null), sessionCookieOptions());
  return response;
}
