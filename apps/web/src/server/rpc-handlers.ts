...
const methodPolicy: Record<string, Route> = {
  ...
  placeDevice: session(async (current, input) => asReply((await import("./smart-home")).placeDevice(current, input))),
  floorPlan: session(async (current, input) => asReply((await import("./smart-home")).floorPlanFor(current, input.unitId))),
  runHomeAction: session(async (current, input) => asReply(await (await import("./smart-home")).runHomeAction(current, input))),
  pairGateway: staff("devices.edit", (actor, input) => import("./gateway-channel").then((mod) => mod.pairGateway(actor, input.gatewayId))),
  ...
  closeObjectPoint: staff("access.gate.open", (actor, input) => closeObjectPointFor(actor, input.objectId, input.pointId)),
  createAccessPoint: staff("access.points.manage", (actor, input) => createAccessPoint(actor, { objectId: input.objectId, name: input.name, api: input.api }), 201),
  updateAccessPoint: staff("access.points.manage", (actor, input) => updateAccessPoint(actor, { objectId: input.objectId, pointId: input.pointId, name: input.name, api: input.api })),
